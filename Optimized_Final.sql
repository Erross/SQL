WITH
/*
    Query_with_OV_File - fixed machine-result mapping, optimized anchors
    ...
    Optimization-only changes in this version, intended to preserve output:
      A. Reuses task_map_base.sample_count in task_samples instead of recalculating REGEXP_COUNT.
      B. Adds valid_proc_execs / valid_sample_raw_ids / valid_runsets anchor CTEs.
      C. Scopes expensive EAV and equipment parameter scans to those anchors.
      D. Adds MATERIALIZE hints to small anchor CTEs where Oracle may otherwise inline/re-scan.
*/
...
valid_sample_raw_ids AS (
    SELECT /*+ MATERIALIZE */ DISTINCT
        s.id AS sample_raw_id,
        s.sample_id
    FROM valid_process_samples vps
    JOIN hub_owner.sam_sample s
        ON s.sample_id = vps.sample_id
),

valid_runsets AS (
    SELECT /*+ MATERIALIZE */ DISTINCT
        runset_id AS runset_raw_id
    FROM task_map
),
...
item_state_sample_flags AS (
    ...
    JOIN valid_proc_execs vpe
        ON vpe.proc_exec_id = pee.parent_id
...
sample_properties AS (
    ...
    JOIN valid_sample_raw_ids vs
        ON vs.sample_raw_id = oi.object_id
...
runset_properties AS (
    ...
    JOIN hub_owner.cor_object_identity oi
        ON oi.class_identity_id = ci.id
    JOIN valid_runsets vr
        ON vr.runset_raw_id = oi.object_id
    JOIN hub_owner.cor_property_value pv
        ON pv.object_identity_id = oi.id
    JOIN hub_owner.cor_property p
        ON p.name = pv.property_id
