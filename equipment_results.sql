...
SELECT 
    r.RUNSET_ID,
    s.SAMPLE_ID,
    df.NAME as CHARACTERISTIC_NAME,
    u.NAME as UNIT,
    pv.VALUE_NUMERIC,
    pv.VALUE_STRING
FROM COR_PARAMETER_VALUE pv
...
SELECT 
    df.NAME as FIELD_NAME,
    df.DISPLAY_ORDER,
    ROW_NUMBER() OVER (PARTITION BY df.DATA_PACKET_ID ORDER BY df.DISPLAY_ORDER) - 1 as CALCULATED_POSITION
FROM RES_MEASUREMENT m
JOIN RES_EQUIPMENT eq ON eq.ID = m.EQUIPMENT_ID
JOIN RES_EQUIPMENT_TYPE et ON et.ID = eq.EQUIPMENT_TYPE_ID
JOIN RES_DATA_PACKET dp ON dp.ID = et.DATA_PACKET_ID
...
JOIN RES_MEASUREMENTSAMPLE ms ON ms.CONTEXT_ID = ctx.ID
WHERE ms.SAMPLE_ID = 'S001'
ORDER BY peep.SOURCE_POSITION, pv.ITEM_INDEX;

--joinshizzle
SELECT 
    r.RUNSET_ID,
    s.SAMPLE_ID,
    pv_name.VALUE_STRING as CHARACTERISTIC_NAME,
    pv_value.VALUE_NUMERIC
FROM COR_PARAMETER_VALUE pv_value
JOIN PEX_PROC_ELEM_EXEC_PARAM peep_value ON peep_value.ID = pv_value.PARENT_IDENTITY
JOIN PEX_PROC_ELEM_EXEC pee ON pee.ID = peep_value.PARENT_ID
JOIN PEX_PROC_ELEM_EXEC_PARAM peep_name ON peep_name.PARENT_ID = pee.ID 
    AND peep_name.SOURCE_POSITION = peep_value.SOURCE_POSITION
JOIN COR_PARAMETER_VALUE pv_name ON pv_name.PARENT_IDENTITY = peep_name.ID
    AND pv_name.VALUE_STRING IS NOT NULL
JOIN PEX_PROC_EXEC pe ON pe.ID = pee.PARENT_ID
JOIN RES_RETRIEVAL_CONTEXT ctx ON ctx.CONTEXT = 
...
JOIN REQ_TASK t ON INSTR(',' || t.SAMPLE_LIST || ',', ',' || s.SAMPLE_ID || ',') > 0
JOIN REQ_RUNSET r ON r.ID = t.RUNSET_ID
WHERE pv_value.VALUE_NUMERIC IS NOT NULL
...
WHERE r.RUNSET_ID = 'TP002'
AND s.SAMPLE_ID = 'S001'
GROUP BY r.RUNSET_ID, s.SAMPLE_ID, peep.ID, peep.SOURCE_POSITION
HAVING MAX(CASE WHEN pv.VALUE_NUMERIC IS NOT NULL THEN pv.VALUE_NUMERIC END) IS NOT NULL;

-- getting ufll name of shit?

SELECT 
    r.RUNSET_ID,
    s.SAMPLE_ID,
    CASE 
        WHEN u.NAME IS NOT NULL THEN MAX(CASE WHEN pv.VALUE_STRING IS NOT NULL THEN pv.VALUE_STRING END) || ' [' || u.NAME || ']'
        ELSE MAX(CASE WHEN pv.VALUE_STRING IS NOT NULL THEN pv.VALUE_STRING END)
...
JOIN RES_EQUIPMENT_TYPE et ON et.ID = eq.EQUIPMENT_TYPE_ID
JOIN RES_DATA_PACKET dp ON dp.ID = et.DATA_PACKET_ID
LEFT JOIN RES_DATA_FIELD df ON df.DATA_PACKET_ID = dp.ID 
    AND df.NAME = pv.VALUE_STRING
LEFT JOIN COR_UNIT u ON u.ID = df.UNIT_ID
WHERE r.RUNSET_ID = 'TP002'
AND s.SAMPLE_ID = 'S001'
GROUP BY r.RUNSET_ID, s.SAMPLE_ID, peep.ID, peep.SOURCE_POSITION, u.NAME
HAVING MAX(CASE WHEN pv.VALUE_NUMERIC IS NOT NULL THEN pv.VALUE_NUMERIC END) IS NOT NULL;