...
SELECT
    ...
    -- Sample Status: check all periodic positions ITEM_INDEX+1, ITEM_INDEX+1+N, ITEM_INDEX+1+2N...
    -- where N = number of samples in SAMPLE_LIST. Any 'X' = abandoned.
    (SELECT CASE
                WHEN MAX(CASE WHEN
                         SUBSTR(pee2.ITEM_STATES,
                                pv.ITEM_INDEX + 1 + (LEVEL - 1) * (REGEXP_COUNT(rt.SAMPLE_LIST, ',') + 1),
     ...
     CONNECT BY LEVEL <= CEIL(LENGTH(pee2.ITEM_STATES) / (REGEXP_COUNT(rt.SAMPLE_LIST, ',') + 1))
            AND PRIOR pee2.ID = pee2.ID
...
SELECT
    ...
    -- Sample Status: same periodic logic using meas_s.ROW_INDEX instead of pv.ITEM_INDEX
    (SELECT CASE                                                                     -- 3  Sample Status
                WHEN MAX(CASE WHEN
                         SUBSTR(pee2.ITEM_STATES,
                                meas_s.ROW_INDEX + 1 + (LEVEL - 1) * (REGEXP_COUNT(rt.SAMPLE_LIST, ',') + 1),
                                1) = 'X' THEN 1 END) = 1 THEN 'abandoned'
                WHEN MAX(CASE WHEN
                         SUBSTR(pee2.ITEM_STATES,
                                meas_s.ROW_INDEX + 1 + (LEVEL - 1) * (REGEXP_COUNT(rt.SAMPLE_LIST, ',') + 1),
    ...
    (SELECT MAX(pee2.ITEM_STATES)                                                    -- 25 Item States (Debug)
     FROM hub_owner.PEX_PROC_ELEM_EXEC pee2
     WHERE pee2.PARENT_ID = pe.ID
       AND pee2.ITEM_STATES IS NOT NULL
       AND pee2.ITEM_STATES NOT LIKE '%\_%' ESCAPE '\'
