...
SELECT
    ...
    -- Sample Status: scan ALL pee rows - same reasoning as manual section.
    -- meas_s.ROW_INDEX is 0-based absolute batch position; +1 for 1-based SUBSTR.
    -- X in ANY pee row at this position = abandoned; D (no X) = completed.
    (SELECT CASE                                                                     -- 3  Sample Status
                WHEN MAX(CASE WHEN UPPER(SUBSTR(pee2.ITEM_STATES, meas_s.ROW_INDEX + 1, 1)) = 'X' THEN 1 END) = 1 THEN 'abandoned'
                WHEN MAX(CASE WHEN UPPER(SUBSTR(pee2.ITEM_STATES, meas_s.ROW_INDEX + 1, 1)) = 'D' THEN 1 END) = 1 THEN 'completed'
                ELSE s.LIFE_CYCLE_STATE
            END
