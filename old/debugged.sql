...
SELECT
    ...
    -- Sample Status: use SOURCE_POSITION=0 (acceptance/sign-off) pee row only.
    -- Absolute batch position = count of co-run samples with smaller SAMPLE_ID + 1.
    -- Samples within a PEX_PROC_EXEC are ordered by SAMPLE_ID (confirmed via audit trail).
    -- SOURCE_POSITION=1 (data collection) always ends all-D and must NOT be used.
    (SELECT CASE pos_char
                WHEN 'X' THEN 'abandoned'
                WHEN 'D' THEN 'completed'
                ELSE s.LIFE_CYCLE_STATE
            END
     FROM (
         SELECT UPPER(SUBSTR(pee2.ITEM_STATES,
             (SELECT COUNT(*) + 1
...
SELECT
    ...
    -- Sample Status: SOURCE_POSITION=0 (acceptance/sign-off) pee only.
    -- meas_s.ROW_INDEX is 0-based absolute batch position; +1 for 1-based SUBSTR.
    -- No periodic formula needed - ROW_INDEX IS the absolute position.
    (SELECT CASE UPPER(SUBSTR(pee2.ITEM_STATES, meas_s.ROW_INDEX + 1, 1))           -- 3  Sample Status
                WHEN 'X' THEN 'abandoned'
                WHEN 'D' THEN 'completed'
                ELSE s.LIFE_CYCLE_STATE
            END
    ...
    (SELECT pee2.ITEM_STATES                                                         -- 25 Item States (Debug)
     FROM hub_owner.PEX_PROC_ELEM_EXEC pee2
     WHERE pee2.PARENT_ID = pe.ID
       AND pee2.SOURCE_POSITION = 0
       AND pee2.ITEM_STATES IS NOT NULL
