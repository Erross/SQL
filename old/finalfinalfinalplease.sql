...
JOIN hub_owner.COR_PARAMETER_VALUE pv
    ...
    AND pv.ITEM_INDEX = (
            -- pv.ITEM_INDEX is a global counter across ALL measurement batches in this PEE.
            -- meas_s.ROW_INDEX resets to 0 per batch, so the correct ITEM_INDEX is:
            --   ROW_INDEX + count of meas_s rows in batches measured before this one.
            -- "Earlier" batches = smaller m.LAST_UPDATED for the same ctx.
            meas_s.ROW_INDEX + (
                SELECT COUNT(*)
                FROM hub_owner.RES_MEASUREMENTSAMPLE ms2
                JOIN hub_owner.RES_MEASUREMENT m2
                     ON m2.ID = ms2.MEASUREMENT_ID
                WHERE ms2.CONTEXT_ID = ctx.ID
                  AND m2.LAST_UPDATED < m.LAST_UPDATED
            )
        )
LEFT JOIN hub_owner.SAM_SAMPLE ms
     ON s.MASTER_SAMPLE_ID = ms.ID
