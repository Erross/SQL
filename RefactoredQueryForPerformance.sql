...
/* keep dynamic result detection, but remove weight-like fields */
...
manual_results AS (
  SELECT DISTINCT
    ...
    CASE
      WHEN tm.task_status = 'completed' THEN 'completed'
      WHEN tm.task_status = 'released' THEN 'released'
      ELSE s.life_cycle_state
    END AS "Sample Status",
